...
functionExpression
	: 'function' LT* Identifier? LT* formalParameterList LT* functionBody ->
        ^( FUNCTION_EXPR formalParameterList functionBody)
	;
...
functionBody
	: '{' LT* sourceElements? LT* '}' -> ^( FUNCTION_BODY sourceElements?)
	;
...
// fragment DjangoVariableCharacter
// 	: ~(('}' '}') | '\\' | LT)
// 	| '\\' EscapeSequence
// 	;
...
variableDeclaration
	: Identifier LT* initialiser -> ^( VARIABLE_DECL Identifier initialiser)
	| Identifier LT* -> ^( VARIABLE_DECL Identifier)
	;

variableDeclarationNoIn
	: Identifier LT* initialiserNoIn? ->
        ^( VARIABLE_DECL Identifier initialiserNoIn)
	;
...
ifStatement
	: 'if' LT!* '(' LT!* expression LT!* ')' LT!* (statement | statementBlock) (LT!* 'else' LT!* (statement | statementBlock))?
	;
...
doWhileStatement
	: 'do' LT!* (statement | statementBlock) LT!* 'while' LT!* '(' expression ')' LT!* ';'!
	;

whileStatement
	: 'while' LT!* '(' LT!* expression LT!* ')' LT!* (statement | statementBlock)
	;

forStatement
	: 'for' LT!* '(' (LT!* forStatementInitialiserPart)? LT!* ';'! (LT!* expression)? LT!* ';'! (LT!* expression)? LT!* ')' LT!* (statement | statementBlock)
	;
...
forInStatement
	: 'for' LT!* '(' LT!* forInStatementInitialiserPart LT!* 'in' LT!* expression LT!* ')' LT!* (statement | statementBlock)
	;
...
withStatement
	: 'with' LT!* '(' LT!* expression LT!* ')' LT!* (statement | statementBlock)
	;
...
assignmentExpression
    ...
    | unaryExpression
    | newExpression -> ^( ASSIGNMENT_EXPR newExpression)
    ;
...
callExpression
	: memberExpression LT* arguments (LT* callExpressionSuffix)* ->
        ^(CALL ^(CALL_IDENTIFIER memberExpression) arguments callExpressionSuffix*)
	;
...
propertyReferenceSuffix
	: '.' LT* Identifier -> ^( MEMBER Identifier)
	;
...
conditionalExpression
	: operatorExpression (LT!* '?' LT!* operatorExpression LT!* ':' LT!* operatorExpression)?
	;
